.class public final Lio/appmetrica/analytics/impl/rd;
.super Ljava/lang/Object;
.source "SourceFile"


# static fields
.field public static final a:Lio/appmetrica/analytics/impl/rd;

.field public static final b:Ljava/util/LinkedHashMap;

.field public static final c:Ljava/lang/String;


# direct methods
.method static constructor <clinit>()V
    .locals 3

    new-instance v0, Lio/appmetrica/analytics/impl/rd;

    invoke-direct {v0}, Lio/appmetrica/analytics/impl/rd;-><init>()V

    sput-object v0, Lio/appmetrica/analytics/impl/rd;->a:Lio/appmetrica/analytics/impl/rd;

    .line 1
    new-instance v0, Ljava/util/LinkedHashMap;

    invoke-direct {v0}, Ljava/util/LinkedHashMap;-><init>()V

    sput-object v0, Lio/appmetrica/analytics/impl/rd;->b:Ljava/util/LinkedHashMap;

    .line 2
    const-string v0, "7.3.0"

    const-string v1, "50119713"

    const-string v2, "io.appmetrica.analytics"

    invoke-static {v2, v0, v1}, Lio/appmetrica/analytics/coreutils/internal/network/UserAgent;->getFor(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v0

    .line 3
    sput-object v0, Lio/appmetrica/analytics/impl/rd;->c:Ljava/lang/String;

    return-void
.end method

.method public constructor <init>()V
    .locals 0

    .line 1
    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    return-void
.end method

.method public static final a(Lio/appmetrica/analytics/impl/h5;)Lio/appmetrica/analytics/networktasks/internal/NetworkTask;
    .locals 19

    move-object/from16 v1, p0

    .line 133
    new-instance v10, Lio/appmetrica/analytics/networktasks/internal/AESRSARequestBodyEncrypter;

    invoke-direct {v10}, Lio/appmetrica/analytics/networktasks/internal/AESRSARequestBodyEncrypter;-><init>()V

    .line 134
    new-instance v2, Lio/appmetrica/analytics/impl/ug;

    invoke-direct {v2, v10}, Lio/appmetrica/analytics/impl/ug;-><init>(Lio/appmetrica/analytics/networktasks/internal/RequestBodyEncrypter;)V

    .line 135
    new-instance v3, Lio/appmetrica/analytics/impl/tb;

    invoke-direct {v3, v1}, Lio/appmetrica/analytics/impl/tb;-><init>(Lio/appmetrica/analytics/impl/h5;)V

    .line 136
    new-instance v18, Lio/appmetrica/analytics/networktasks/internal/NetworkTask;

    .line 137
    new-instance v12, Lio/appmetrica/analytics/coreutils/internal/executors/BlockingExecutor;

    invoke-direct {v12}, Lio/appmetrica/analytics/coreutils/internal/executors/BlockingExecutor;-><init>()V

    .line 138
    new-instance v13, Lio/appmetrica/analytics/impl/v9;

    .line 139
    iget-object v0, v1, Lio/appmetrica/analytics/impl/h5;->a:Landroid/content/Context;

    .line 140
    invoke-direct {v13, v0}, Lio/appmetrica/analytics/impl/v9;-><init>(Landroid/content/Context;)V

    .line 141
    new-instance v14, Lio/appmetrica/analytics/networktasks/internal/AllHostsExponentialBackoffPolicy;

    sget-object v0, Lio/appmetrica/analytics/impl/rd;->a:Lio/appmetrica/analytics/impl/rd;

    sget-object v4, Lio/appmetrica/analytics/impl/pd;->a:Lio/appmetrica/analytics/impl/pd;

    invoke-virtual {v0, v4}, Lio/appmetrica/analytics/impl/rd;->a(Lio/appmetrica/analytics/impl/pd;)Lio/appmetrica/analytics/networktasks/internal/ExponentialBackoffDataHolder;

    move-result-object v0

    invoke-direct {v14, v0}, Lio/appmetrica/analytics/networktasks/internal/AllHostsExponentialBackoffPolicy;-><init>(Lio/appmetrica/analytics/networktasks/internal/ExponentialBackoffDataHolder;)V

    .line 142
    new-instance v15, Lio/appmetrica/analytics/impl/Pg;

    .line 146
    new-instance v4, Lio/appmetrica/analytics/networktasks/internal/FullUrlFormer;

    invoke-direct {v4, v2, v3}, Lio/appmetrica/analytics/networktasks/internal/FullUrlFormer;-><init>(Lio/appmetrica/analytics/networktasks/internal/IParamsAppender;Lio/appmetrica/analytics/networktasks/internal/ConfigProvider;)V

    .line 147
    new-instance v5, Lio/appmetrica/analytics/networktasks/internal/RequestDataHolder;

    invoke-direct {v5}, Lio/appmetrica/analytics/networktasks/internal/RequestDataHolder;-><init>()V

    .line 148
    new-instance v6, Lio/appmetrica/analytics/networktasks/internal/ResponseDataHolder;

    .line 149
    new-instance v0, Lio/appmetrica/analytics/networktasks/internal/DefaultResponseValidityChecker;

    invoke-direct {v0}, Lio/appmetrica/analytics/networktasks/internal/DefaultResponseValidityChecker;-><init>()V

    .line 150
    invoke-direct {v6, v0}, Lio/appmetrica/analytics/networktasks/internal/ResponseDataHolder;-><init>(Lio/appmetrica/analytics/networktasks/internal/ResponseValidityChecker;)V

    .line 151
    invoke-virtual/range {p0 .. p0}, Lio/appmetrica/analytics/impl/h5;->h()Lio/appmetrica/analytics/impl/M6;

    move-result-object v7

    .line 152
    invoke-virtual/range {p0 .. p0}, Lio/appmetrica/analytics/impl/h5;->o()Lio/appmetrica/analytics/logger/appmetrica/internal/PublicLogger;

    move-result-object v8

    .line 153
    invoke-virtual/range {p0 .. p0}, Lio/appmetrica/analytics/impl/h5;->u()Lio/appmetrica/analytics/impl/wn;

    move-result-object v9

    move-object v0, v15

    .line 154
    invoke-direct/range {v0 .. v10}, Lio/appmetrica/analytics/impl/Pg;-><init>(Lio/appmetrica/analytics/impl/h5;Lio/appmetrica/analytics/impl/ug;Lio/appmetrica/analytics/impl/tb;Lio/appmetrica/analytics/networktasks/internal/FullUrlFormer;Lio/appmetrica/analytics/networktasks/internal/RequestDataHolder;Lio/appmetrica/analytics/networktasks/internal/ResponseDataHolder;Lio/appmetrica/analytics/impl/M6;Lio/appmetrica/analytics/logger/appmetrica/internal/PublicLogger;Lio/appmetrica/analytics/impl/wn;Lio/appmetrica/analytics/networktasks/internal/RequestBodyEncrypter;)V

    .line 155
    new-instance v0, Lio/appmetrica/analytics/impl/jn;

    invoke-direct {v0}, Lio/appmetrica/analytics/impl/jn;-><init>()V

    .line 156
    invoke-static {v0}, Lkotlin/collections/CollectionsKt__CollectionsJVMKt;->listOf(Ljava/lang/Object;)Ljava/util/List;

    move-result-object v16

    .line 157
    sget-object v17, Lio/appmetrica/analytics/impl/rd;->c:Ljava/lang/String;

    move-object/from16 v11, v18

    .line 158
    invoke-direct/range {v11 .. v17}, Lio/appmetrica/analytics/networktasks/internal/NetworkTask;-><init>(Ljava/util/concurrent/Executor;Lio/appmetrica/analytics/coreapi/internal/io/IExecutionPolicy;Lio/appmetrica/analytics/networktasks/internal/ExponentialBackoffPolicy;Lio/appmetrica/analytics/networktasks/internal/UnderlyingNetworkTask;Ljava/util/List;Ljava/lang/String;)V

    return-object v18
.end method


# virtual methods
.method public final declared-synchronized a(Lio/appmetrica/analytics/impl/pd;)Lio/appmetrica/analytics/networktasks/internal/ExponentialBackoffDataHolder;
    .locals 4

    monitor-enter p0

    .line 1
    :try_start_0
    sget-object v0, Lio/appmetrica/analytics/impl/rd;->b:Ljava/util/LinkedHashMap;

    .line 62
    invoke-virtual {v0, p1}, Ljava/util/LinkedHashMap;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v1

    if-nez v1, :cond_0

    .line 63
    new-instance v1, Lio/appmetrica/analytics/networktasks/internal/ExponentialBackoffDataHolder;

    .line 64
    new-instance v2, Lio/appmetrica/analytics/impl/ua;

    .line 65
    sget-object v3, Lio/appmetrica/analytics/impl/la;->C:Lio/appmetrica/analytics/impl/la;

    .line 66
    invoke-virtual {v3}, Lio/appmetrica/analytics/impl/la;->w()Lio/appmetrica/analytics/impl/re;

    move-result-object v3

    .line 67
    invoke-direct {v2, v3, p1}, Lio/appmetrica/analytics/impl/ua;-><init>(Lio/appmetrica/analytics/impl/re;Lio/appmetrica/analytics/impl/pd;)V

    .line 68
    invoke-direct {v1, v2}, Lio/appmetrica/analytics/networktasks/internal/ExponentialBackoffDataHolder;-><init>(Lio/appmetrica/analytics/networktasks/internal/HostRetryInfoProvider;)V

    .line 131
    invoke-interface {v0, p1, v1}, Ljava/util/Map;->put(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;

    goto :goto_0

    :catchall_0
    move-exception p1

    goto :goto_1

    .line 132
    :cond_0
    :goto_0
    check-cast v1, Lio/appmetrica/analytics/networktasks/internal/ExponentialBackoffDataHolder;
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    monitor-exit p0

    return-object v1

    :goto_1
    monitor-exit p0

    throw p1
.end method
